//go:generate mockgen -destination=rocket_mocks_test.go -package=rocket github.com/muhammaddipo/grpc-services/tree/dev/internal/rocket Store

package rocket

import "context"

// Rocket - Should containt the definition of our rocket
type Rocket struct {
	ID      string
	Name    string
	Type    string
...
// our database inmplementation to follow
...
// Service - our rocket service, responsbile for
// updating the rocket invetory
type Service struct {
	Store Store
}
...
// GetRocketByID - Retrjeves a rocket based on the ID from the store
